...
class Signal_processing():
    ...
    def extract_color(self, ROIs):
        ...
        for ROI in ROIs:
            b.append(np.mean(ROI[:,:,0]))
            g.append(np.mean(ROI[:,:,1]))
            r.append(np.mean(ROI[:,:,2]))

        #b = np.mean(ROI[:,:,2])
        #return r, g, b
        output_val_red = np.mean(r)
        output_val_green = np.mean(g)
        output_val_blue = np.mean(b)


        return output_val_blue,output_val_green,output_val_red
    ...
    def fft(self, data_buffer, fps):
        ...
        print('freqs_in_minute',freqs_in_minute)
