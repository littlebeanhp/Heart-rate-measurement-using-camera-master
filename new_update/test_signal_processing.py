...
if __name__ == "__main__":
    ...
    while True:
        ...
        if(len(aligned_shape)==68):
            cv2.rectangle(aligned_face,(aligned_shape[20][0], aligned_shape[20][1]), #draw rectangle on right and left cheeks
                    (aligned_shape[23][0],aligned_shape[27][1]), (0,255,0), 0)
            cv2.rectangle(aligned_face, (aligned_shape[27][0]-16, aligned_shape[27][1]-16),
                    (aligned_shape[27][0]+16,aligned_shape[27][1]+16), (0,255,0), 0)
        else:
            #print(shape[4][1])
            #print(shape[2][1])
        ...
        blue_val,green_val,red_val = sp.extract_color(ROIs)
        print("Greenvalue:",green_val)
        
        data_buffer.append(green_val)
        sumdata_buffer.append([blue_val,green_val,red_val])

        print(sumdata_buffer)
